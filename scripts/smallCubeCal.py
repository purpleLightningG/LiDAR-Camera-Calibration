...
def get_camera_plane_and_pose(corners_ud, K, tag_size):
    ...
    ret, rvec, tvec = cv2.solvePnP(
        objp, corners_ud.astype(np.float32), K, distCoeffs[:5]
    )
    if not ret:
...
def find_lidar_plane_guided(
    pts_lidar, T_l2c_initial, corners_ud, K, img_for_debug, frame_name
):
    """
    ...
    print(
        f"    • Filtered to {len(pts_on_target)} LiDAR points within expanded search area."
    )
...
def calibrate():
    ...
    for rgb_p, pcd_p, name in frames:
        ...
        print(
            f"    • Generated initial guess for translation: {np.round(initial_t, 3)}"
        )

        n_l, d_l = find_lidar_plane_guided(
            pts, T_initial_guess, corners_ud, K, img, name
        )
