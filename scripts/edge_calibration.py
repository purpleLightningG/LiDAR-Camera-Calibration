...
def list_paired_frames(rgb_folder, pcd_folder):
    ...
    rgbs = sorted([f for f in os.listdir(rgb_folder) if f.lower().endswith(('.png', '.jpg'))])
    pcds = sorted([f for f in os.listdir(pcd_folder) if f.lower().endswith('.pcd')])
    common_names = sorted(list(set(os.path.splitext(f)[0] for f in rgbs) & set(os.path.splitext(f)[0] for f in pcds)))
    pairs = []
...
def detect_apriltag_pose(gray):
    ...
    tags = det.detect(gray, estimate_tag_pose=True, camera_params=[FX, FY, CX, CY], tag_size=TAG_SIZE)
    for t in tags:
        if t.tag_id == TAG_ID:
            R = t.pose_R
...
def find_planes_in_manual_region(pcd):
    ...
    for _ in range(3):
        if len(remaining_pts) < MIN_PLANE_INLIERS: break
        
        temp_pcd = o3d.geometry.PointCloud()
        temp_pcd.points = o3d.utility.Vector3dVector(remaining_pts)
        
        plane_model, inlier_indices = temp_pcd.segment_plane(RANSAC_DIST, 3, RANSAC_ITERS)
        
        if len(inlier_indices) < MIN_PLANE_INLIERS: break
        
        plane_inliers = remaining_pts[inlier_indices]
        
        # Validate the size of the found plane
        try:
            obb = o3d.geometry.OrientedBoundingBox.create_from_points(o3d.utility.Vector3dVector(plane_inliers))
            extents = sorted(obb.extent)
            
            if abs(extents[1] - CUBE_SIDE_M) < CUBE_TOL_M and abs(extents[2] - CUBE_SIDE_M) < CUBE_TOL_M:
...
def visualize_candidate_planes(full_pcd, candidates):
    ...
    o3d.visualization.draw_geometries(geometries, window_name="Select the plane that corresponds to the AprilTag")
...
def main():
    ...
    for rgb_path, pcd_path, name in frames:
        ...
        try:
            sel = input("Enter the index of the correct plane (or press Enter to skip): ").strip()
            if not sel:
