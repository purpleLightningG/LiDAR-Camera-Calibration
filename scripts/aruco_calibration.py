    ...
    if ids is not None:
        for i, marker_id in enumerate(ids):
            if marker_id[0] == ARUCO_ID:
                ...
                board_object_pts = get_board_corners_in_object_frame(dx=0.040, dy=-0.030)
                ...
                img_corners, _ = cv2.projectPoints(board_object_pts, rvec, tvec, K, DIST_COEFFS)
                ...
                if manual_corners:
                    ...
                    print("Please click the 4 actual A4 paper corners (order: TL, TR, BR, BL). Press ESC to abort.")
                    ...
                    return board_corners_cam, computed_img_corners, clicked_points, False
                else:
...
def show_plane_result(pcd, plane_points):
    ...
    o3d.visualization.draw_geometries([pcd, plane_pcd], window_name="RANSAC Plane Detection")

# MAIN SCRIPT
if __name__ == "__main__":
    image_files = sorted([f for f in os.listdir(os.path.join(DATA_PATH, 'rgb')) if f.endswith('.jpg')])
    all_cam_corners = []
    all_lidar_corners = []
    for image_name in image_files:
        ...
        cam_corners, projected_corners, clicked_corners, is_rejected = get_aruco_in_image(image, manual_corners=True)
        if is_rejected or cam_corners is None:
            print("  > ⚠️ Tag or board not detected in camera or frame rejected by user. Skipping.")
            continue
        ...
        if len(points) < 100:
            ...
            continue
        plane_params, plane_points = ransac_plane(points, threshold=0.01, iterations=400)
        print(f"  > Plane equation: {np.round(plane_params, 4)}. Inliers: {len(plane_points)}")
    ...
    corres = o3d.utility.Vector2iVector(np.array([np.arange(len(source)), np.arange(len(source))]).T)
