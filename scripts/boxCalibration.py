...
# ─── USER SETTINGS ─────────────────────────────────────────────────────────────
RGB_FOLDER    = r"C:/Users/Dream Team/Desktop/recorded_data/desired/rgb"
PCD_FOLDER    = r"C:/Users/Dream Team/Desktop/recorded_data/desired/lidar"
TAG_ID        = 8
TAG_SIZE      = 0.156   # meters (outer black square size, 15.6 cm)
...
def face_extents_minarect(inliers_xyz, plane_normal):
    ...
    u = np.cross(n, axis); u /= (np.linalg.norm(u) + 1e-12)
    v = np.cross(n, u)
    ...
    if P2_unique.shape[0] < 4 or (spread_u < 1e-4 and spread_v < 1e-4):
        ...
        return 0.0, 0.0, thickness, {"reason":"degenerate_2d", "spread_u":spread_u, "spread_v":spread_v}
    ...
    dbg = {"spread_u":spread_u, "spread_v":spread_v, "unique":int(P2_unique.shape[0])}
...
def calibrate():
    ...
    for rgb_p, pcd_p, name in frames:
        ...
        alpha_deg = float(np.degrees(np.arccos(np.clip(n_c[2], -1, 1))))  # approx tilt to optical axis
        if px_side < MIN_PX_SIDE or alpha_deg > ANGLE_DEG_MAX:
            print(f" dropped (px_side={px_side:.1f} < {MIN_PX_SIDE} or alpha={alpha_deg:.1f}° > {ANGLE_DEG_MAX}°)")
            continue
        ...
        for n_l,d_l,inl in all_cands:
            ...
            face_ok = (
                (abs(short - min(FACE_W, FACE_H)) < FACE_TOL and
                 abs(long  - max(FACE_W, FACE_H)) < FACE_TOL) or
                (abs(short - max(FACE_W, FACE_H)) < FACE_TOL and
                 abs(long  - min(FACE_W, FACE_H)) < FACE_TOL)
            )
            thickness_ok = (thick < 0.05)
            if face_ok and thickness_ok and short > 1e-4 and long > 1e-4:
                filtered.append((n_l,d_l,inl))
            else:
                print(f"    face check -> short={short:.3f} long={long:.3f} thick={thick:.3f}  "
    ...
    def residuals(x, R_ref, t_ref):
        ...
        if BASELINE_PRIOR_M > 0:
            res.append( np.array([(np.linalg.norm(t) - BASELINE_PRIOR_M)/BASELINE_PRIOR_SIGMA]) )
        return np.concatenate(res)

    x0 = pack(R0, t0)
    sol = least_squares(residuals, x0, args=(R0, t0), verbose=1, xtol=1e-9, ftol=1e-9, gtol=1e-9, max_nfev=200)
